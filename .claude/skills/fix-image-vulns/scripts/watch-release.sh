#!/bin/bash
# 步骤 3b：监控当前 HEAD 提交触发的 "Alauda Release" 流水线 run
# 用法: watch-release.sh [分支名]（缺省为当前分支）
# 与"看分支最新 run"不同，本脚本按 headSha 精确匹配：迭代修复会在同一分支多次
# push，每次都触发新 run，只看最新一条可能误拿上一轮的旧 run。
# 环境变量:
#   FIX_WATCH_INTERVAL        轮询间隔秒数，默认 30
#   FIX_WATCH_TIMEOUT         流水线完成等待上限秒数，默认 2400（40 分钟）
#   FIX_WATCH_APPEAR_TIMEOUT  等待 run 出现的上限秒数，默认 180
# 退出码: 0=成功（输出构建镜像名） 1=前置失败 2=流水线失败（附失败日志摘要）
#         3=等待超时 4=未发现本次提交的 run
...
TIMEOUT="${FIX_WATCH_TIMEOUT:-2400}"
APPEAR_TIMEOUT="${FIX_WATCH_APPEAR_TIMEOUT:-180}"
...
echo "==> 等待分支 ${BRANCH} 上 HEAD=${HEAD_SHA:0:10} 的 '${WORKFLOW}' run 出现（最长 ${APPEAR_TIMEOUT}s）..."
START="$(date +%s)"
RUN_ID=""
while :; do
  RUN_ID="$(gh run list --repo "${REPO_SLUG}" --workflow "${WORKFLOW}" --branch "${BRANCH}" --limit 10 \
    --json databaseId,headSha \
    --jq "[.[] | select(.headSha == \"${HEAD_SHA}\")][0].databaseId // empty" 2>/dev/null || true)"
  [[ -n "${RUN_ID}" ]] && break
  if (( $(date +%s) - START > APPEAR_TIMEOUT )); then
    echo "==> RESULT: PIPELINE_NOT_FOUND 在 ${APPEAR_TIMEOUT}s 内没有发现本次提交的 '${WORKFLOW}' run"
    echo "    可能原因: 1) 尚未 push 或 PR 的 base 分支不是 main（该流水线的 pull_request 触发只针对 main）；"
...
if [[ "${CONCLUSION}" == "success" ]]; then
  # 从 "Output image: <镜像>" 步骤名结构化提取产物镜像
  IMAGE="$(gh run view --repo "${REPO_SLUG}" "${RUN_ID}" --json jobs \
