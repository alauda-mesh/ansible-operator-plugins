...
if PR_INFO="$(gh pr view "${CUR_BRANCH}" --json number,url --jq '"\(.number) \(.url)"' 2>/dev/null)" && [[ -n "${PR_INFO}" ]]; then
  echo "==> 分支 ${CUR_BRANCH} 已存在 PR，直接复用"
else
  TITLE="chore(sync): merge upstream/${UPSTREAM_BRANCH} ($(date +%F))"
  if [[ -n "${BODY_FILE}" ]]; then
    gh pr create --base "${TARGET_BRANCH}" --head "${CUR_BRANCH}" --title "${TITLE}" --body-file "${BODY_FILE}" >/dev/null
  else
    # 兜底正文：合入的上游提交列表
    ...
    gh pr create --base "${TARGET_BRANCH}" --head "${CUR_BRANCH}" --title "${TITLE}" --body "${BODY}" >/dev/null
  fi
  PR_INFO="$(gh pr view "${CUR_BRANCH}" --json number,url --jq '"\(.number) \(.url)"')"
fi
